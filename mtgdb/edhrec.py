...
# https://json.edhrec.com/cards/death-cloud
# https://json.edhrec.com/pages/commanders/year.json
# https://json.edhrec.com/pages/commanders/year-past2years-1.json
# https://json.edhrec.com/pages/commanders/w.json
# https://json.edhrec.com/pages/commanders/w-mono-whitecommanders-1.json
#
...
class EdhRec:
    non_noise = ["name", "sanitized", "sanitized_wo", "cards", "url", "inclusion", "label", "num_decks", "potential_decks", "salt_score", "synergy_score"]
    ...
    async def card_salt(self, year: str = None) -> AsyncGenerator[dict, None]:
        file = "top/salt.json"
        if year:
            file = "top/salt-{}.json".format(year)
        async for (tag, item) in self.get_page(file):
            yield self._build(tag, item)

    async def card_top_by_period(self, selector: str = "year") -> AsyncGenerator[dict, None]:
        if selector == "week":
            file = "top/week.json".format(selector)
            tag = "pastweek"
        elif selector == "month":
            file = "top/month.json".format(selector)
            tag = "pastmonth"
        else:
            file = "top/year.json".format(selector)
            tag = "past2years"
    ...
    async def card_top_by_type(self, type: str, tag: str = "pastweek") -> AsyncGenerator[dict, None]:
        file = "top/{}.json".format(type)
        async for (tag, item) in self.get_page(file, tag):
            yield self._build(tag, item)

    async def card_by_theme(self, theme, selector: str = None, tag: str = None):
        file = "themes/{theme}.json".format(theme=theme)
        if selector:
            file = "themes/{theme}/{selector}.json".format(theme=theme, selector=selector)
        async for (tag, item) in self.get_page(file, tag):
            yield self._build(tag, item)

    async def card_by_set(self, code: str, tag: str = None) -> AsyncGenerator[dict, None]:
        async for (tag, item) in self.get_page("sets/{code}.json".format(code=code), tag):
            yield self._build(tag, item)
    ...
    async def commanders(self, selector: str = None) -> AsyncGenerator[dict, None]:
        ...
        if selector:
            file = "commanders/{}.json".format(selector)
        async for (tag, item) in self.get_page(file):
            yield self._build(tag, item)

    async def combos_by_color(self, color: str) -> AsyncGenerator[dict, None]:
        file = "commanders/{}.json".format(color)
        async for (tag, item) in self.get_page(file):
            yield self._build(tag, item)
    ...
    def _build(self, tag, item):
        ...
        if body.get("url"):
            body["slug"] = body.get("url", "").split("/")[-1]
    ...
    async def get_page(self, file, tag=None) -> AsyncGenerator[Tuple[str, dict], None]:
        url = '{root}/pages/{file}'.format(root=self.root, file=file)
        try:
